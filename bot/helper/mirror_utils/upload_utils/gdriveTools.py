...
class GoogleDriveHelper:
        ...
        try:
            ...
            if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                if SHORTENER is not None and SHORTENER_API is not None:
                    ...
                    buttons.buildbutton("🌺GDrive🌺", surl)
                else:
                ...
                if INDEX_URL is not None:
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        ...
                        buttons.buildbutton("☘️Direct Link☘️", siurl)
                    else:
                        buttons.buildbutton("☘️Direct Link☘️", url)
                ...
                msg += f'<b>Nama File : </b><code>{file.get("name")}</code>'
    ...
    def edit_telegraph(self):
        ...
        for content in self.telegraph_content :
            if nxt_page == 1 :
                content += f'<b><a href="https://telegra.ph/{self.path[nxt_page]}">Selanjutnya</a></b>'
                nxt_page += 1
            else :
                if prev_page <= self.num_of_path:
                    content += f'<b><a href="https://telegra.ph/{self.path[prev_page]}">Sebelumnya</a></b>'
                    prev_page += 1
            ...
            Telegraph(access_token=telegraph_token).edit_page(path = self.path[prev_page],
                                 title = 'Gaming Corner',
                                 author_name='Gaming Corner',
                                 author_url='https://discord.sucks',
    ...
    def drive_list(self, fileName):
        ...
        if response["files"]:
            ...
            if len(self.telegraph_content) == 0:
                return "Tidak ada hasil ditemukan :(", None

            for content in self.telegraph_content :
                self.path.append(Telegraph(access_token=telegraph_token).create_page(
                                                        title = 'Gaming Corner',
                                                        author_name='Gaming Corner Mirror',
                                                        author_url='https://discord.sucks',
                                                        html_content=content
                                                        )['path'])
            ...
            msg = f"<b>Hasil Pencarian {fileName}</b>"
            buttons = button_build.ButtonMaker()   

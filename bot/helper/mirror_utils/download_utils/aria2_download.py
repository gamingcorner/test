...
class AriaDownloadHelper(DownloadHelper):
    ...
    def __onDownloadStarted(self, api, gid):
        ...
        if STOP_DUPLICATE_MIRROR:
            if smsg:
                dl.getListener().onDownloadError(f'😡😡 File ini sudah ada di dalam Drive. Cari sebelum Mirror!\n\n')
                print(dl.getListener())
                sendMarkup(" Ini Filenya:👇👇", dl.getListener().bot, dl.getListener().update, button)
